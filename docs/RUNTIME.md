...
| `subskill` | **Always refuses** — `subskill <id> not resolved in this runtime invocation` | Same — unconditional, in every mode. See [Threat Model](https://github.com/dot-skill/skillerr/wiki/Threat-Model) T4 and [RFC 0004](https://github.com/dot-skill/skillerr/wiki/RFC-0004-Dangling-Step-Kinds) for why this step kind exists in the schema but has no resolution semantics yet |
