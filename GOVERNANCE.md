...
Open `.skill` Protocol is **1.0.0 (Stable)**. Reference packages (`skillerr`, `@skillerr/*`, currently **1.7.0**) are versioned separately and in lockstep; treat `packages/skillerr/package.json` as the single source of truth for that number if it ever drifts from what's restated here.
